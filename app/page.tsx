    ...
    <div style={{ padding: "40px", maxWidth: "1200px", margin: "0 auto" }}>
      ...
      {fetching ? (
        ...
        <>
          ...
          >
            {devices.filter(d => selectedCategory === "Tümü" || d.category === selectedCategory).map((d) => (
              <div key={d.id} className="glass-panel" style={{ display: "flex", flexDirection: "column", overflow: "hidden", transition: "transform 0.2s", opacity: d.stock <= 0 ? 0.6 : 1 }}>
                <div style={{ position: "relative", width: "100%", height: "220px", background: "white" }}>
                  {d.stock <= 0 && (
                ...
                <Image src={d.imageUrl} alt={d.name} fill style={{ objectFit: "contain", padding: "20px" }} />
              </div>
            ...
            {devices.length === 0 && (
              <div style={{ gridColumn: "1 / -1", textAlign: "center", padding: "60px 20px", color: "#666" }}>
                <p style={{ fontSize: "18px", marginBottom: "10px" }}>Henüz hiç cihaz eklenmemiş.</p>
                {role === "admin" && <p>Admin paneline giderek ilk cihazı ekleyebilirsin!</p>}
              </div>
            )}
          </motion.div>
        </>
